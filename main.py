...
class PlakaTespitUygulamasi:
    ...
    def process_video_read(self):
        """Video akışında otomatik okuma (5 kez aynı sonuç gelince göster)."""
        ...
        if self.video_stable_count < self.video_stable_required:
            self.status_hint.config(
                text=f"Okuma stabil değil ({self.video_stable_count}/{self.video_stable_required})"
            )
            return
